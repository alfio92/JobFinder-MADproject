...
export class MyApp {
  ...
  setMenuByUserType(){
    ...
    if( this.usertype === 'user' ){
        ...
        this.userstorage.fetchLoggedUser().then( (user) => {
                    ...
                    this.pages = [
                      {title: 'Home', icon:'home', component: HomePage, parameter: ''},
                      {title: 'Richieste', icon:'news', component: JobrequestlistPage, parameter: this.user.id},
                      {title: 'Logout', icon:'', component: LoginPage, parameter: ''}
                      ];
  ...
  if( this.user != undefined) this.nav.push(ProfilePage, {user: this.user.id, myId: this.user.id});
